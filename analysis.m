...
% Display the result
disp(['Number of state switches: ', num2str(switch_count), ' out of ', num2str(num_cols)]);
disp(switch_locations);
disp(['Number of valid state switches: ', num2str(switch_count_valid), ' out of ', num2str(num_grps)]);
% disp(loc);
disp([num2str(switch_count), ' / ', num2str(switch_count_valid)])

% disp(['Percentage of states switches: ', num2str(all_switch_percentage)]);
disp(['Mean firing rate: ', num2str(mean(rEf(rEf>0)))]);
